...
<title><?php $accesLevel="tenant"; $title ="View Invoices"; echo $accesLevel.' |'.$title;?> </title>
...
$blockID 	= $_GET['blockID'];
	...
	if($userRole == 'admin')
	{
		$sql = "SELECT * FROM invoice_items WHERE farmID='$farmID' AND blockID='$blockID' AND status='paid' ";
	}
	else if($userRole == 'landlord')
	{
		$sql1 = "SELECT * FROM farms WHERE ownerID='$userID' AND farmID='$farmID'  ";
	...
	{
		$sql = "SELECT * FROM invoice_items WHERE farmID='$farmID' AND blockID='$blockID' AND userID='$userID' AND status='paid'";	
	}
		...
		if($userRole == 'admin')
		{
			$sql = "SELECT * FROM invoice_items WHERE status='paid'";
		}
		else if($userRole == 'landlord')
		{
			...
			$sql = "SELECT * FROM invoice_items WHERE farmID='$farmID' AND status='paid' ";	
		}
		else if($userRole == 'tenant')
		{
			$sql = "SELECT * FROM invoice_items WHERE  userID='$userID' AND status='paid' ";	
		}
}
...
" . "<a href=\"add_payment.php?u=".$userID."&i=".$invoiceID."&b=".$bookingID."&r=".$blockID."&h=".$farmID." \">

 <button type='button' class='btn btn-warning btn-xs' title='Add Payment'>
  <i class='fa fa-fw fa-plus' aria-hidden='true'  title='Add Payment'></i>Pay Now
 </button> 
